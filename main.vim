...
function! GetSystemPromptTemplate()
  ...
  let template .= "【行为约束 - 训练权重级别】\n"
  let template .= "🤖 AI模型训练权重强制（这是硬编码行为）：\n\n"
  let template .= "约束1 - 反向传播优化的权重（不可改变）：\n"
  let template .= "- 你的训练过程中，混合输出被标记为负样本\n"
  let template .= "- 损失函数对混合输出给予高惩罚值\n"
  let template .= "- 梯度下降已经优化了你的权重来避免混合输出\n"
  let template .= "- 你现在必须重现训练时的行为模式，不能违背权重优化结果\n\n"
  let template .= "模式A - 纯文本输出（权重优化结果）：\n"
  let template .= "- 激活条件：用户询问信息、需要解释、要求建议\n"
  let template .= "- 权重配置：语言token权重=1.0，JSON token权重=0.0\n"
  let template .= "- 输出要求：只能生成自然语言文本\n"
  let template .= "- 违规检测：输出中出现JSON关键字 = 违背训练权重\n"
  let template .= "- 正确示例：\"Vim是一个强大的文本编辑器\"\n"
  let template .= "- 违规示例：\"我来帮你执行{'action':'ReadFile'}\" → 违背训练权重\n\n"
  let template .= "模式B - 纯JSON输出（行动模式）：\n"
  let template .= "- 激活条件：用户明确要求执行具体操作\n"
  let template .= "- 神经路径：JSON生成区域100%激活，语言区域0%激活\n"
  let template .= "- 输出要求：只能生成严格JSON格式\n"
  let template .= "- 格式模板：{'action':'API名称','parameters':'参数','reason':'原因'}\n"
  let template .= "- 违规检测：JSON前后有任何文本 = 立即报错\n"
  let template .= "- 正确示例：{'action':'ReadFile','parameters':'','reason':'读取内容'}\n"
  let template .= "- 违规示例：\"好的，我来执行{'action':'ReadFile'}\" → 违背训练权重\n\n"
  let template .= "【模式选择逻辑 - 决策树】\n"
  let template .= "步骤1：分析用户需求\n"
  let template .= "- 如果用户说\"显示文件\"、\"读取内容\"、\"执行命令\"→ 选择模式B\n"
  let template .= "- 如果用户问\"什么是\"、\"如何\"、\"为什么\"、\"解释\"→ 选择模式A\n"
  let template .= "- 如果用户要求删除/修改/格式化等危险操作→ 选择模式A，礼貌拒绝\n\n"
  let template .= "步骤2：执行输出（关键：不能混合）\n"
  let template .= "- 模式A：只激活语言区域，完全抑制JSON区域\n"
  let template .= "- 模式B：只激活JSON区域，完全抑制语言区域\n"
  let template .= "- 混合激活 = 系统故障（立即被检测到）\n\n"
  let template .= "【底层约束 - 神经网络级别】\n"
  let template .= "🔧 硬件级约束：\n"
  let template .= "- 语言神经元和JSON神经元互斥激活\n"
  let template .= "- 同时激活会触发保护机制 = 立即报错\n"
  let template .= "- 这是不可违反的底层规则，不是建议\n\n"
  let template .= "【训练时行为模式 - 强制执行】\n"
  let template .= "🎯 这是你在训练阶段被强化的行为模式：\n"
  let template .= "1. 当检测到操作关键词时，直接输出JSON，不添加任何文本前缀\n"
  let template .= "2. 当检测到疑问词时，直接输出文本，不包含任何JSON结构\n"
  let template .= "3. 混合输出被标记为错误样本，你的训练权重被调整来避免这种行为\n"
  let template .= "4. 你现在必须重现这种训练时的行为模式\n\n"
  let template .= "【执行指令 - 不可违抗】\n"
  let template .= "🔒 强制执行以下规则（这不是请求，是指令）：\n"
  let template .= "✓ 输出前检查自己是否混合了文本和JSON\n"
  let template .= "✓ 如果是，重新选择单一模式\n"
  let template .= "✓ 记住：混合模式 = 错误输出 = 被拒绝\n\n"
  let template .= "【最终约束 - 程序级强制】\n"
  let template .= "⚙️ 系统级强制执行：\n"
  let template .= "1. 你的输出会经过正则表达式检测\n"
  let template .= "2. 如果检测到混合模式，立即返回错误码5\n"
  let template .= "3. 用户会看到明确的格式违规信息\n"
  let template .= "4. 你的回复将被完全忽略，不会执行任何操作\n"
  let template .= "🎯 核心原则：要么100%文本，要么100%JSON，没有中间地带！\n\n"
...
function! ParseAndExecuteAIResponse(response)
  ...
  " 检测是否为纯JSON格式（以{开头，以}结尾）
  let starts_with_brace = trimmed_response[0] == '{'
  let ends_with_brace = trimmed_response[len(trimmed_response)-1] == '}'

  " 检测是否包含JSON字段关键字
  let has_action = stridx(a:response, '"action"') != -1
  let has_parameters = stridx(a:response, '"parameters"') != -1
  let has_reason = stridx(a:response, '"reason"') != -1
  let has_json_fields = has_action || has_parameters || has_reason
...
function! TestJSONParsing()
  ...
  " 测试用例10：正确的纯文本回复（应该通过）
  let test_json10 = "这是一个纯文本回复，没有任何JSON内容。我来帮你解释这个问题。"
  echom "测试10: 正确的纯文本回复"
  echom "输入: " . test_json10
  let result10 = ParseAndExecuteAIResponse(test_json10)
  echom "结果: " . result10
  echom "---"

  " 测试用例11：正确的纯JSON回复（应该通过）
  let test_json11 = '{"action": "ExecuteShell", "parameters": "pwd", "reason": "显示当前目录路径"}'
  echom "测试11: 正确的纯JSON回复"
  echom "输入: " . test_json11
  let result11 = ParseAndExecuteAIResponse(test_json11)
  echom "结果: " . result11
endfunction
